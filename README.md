...
- 输入框获得焦点或检测到用户正在输入时会暂停自动切换，避免模型菜单抢走光标；输入结束并离开输入框后再继续应用。
- 如果用户在输入框仍聚焦时直接按回车或点击发送，脚本会先完成一次默认模型检查，再继续原来的发送动作。
...
Gemini 的输入框通常比模型选择栏更早加载。脚本会识别 `contenteditable` 输入框、最近的键盘输入和用户手动打开的模型菜单：输入过程中不会打开模型菜单，也不会覆盖用户刚刚做出的模型选择。输入结束并离开输入框后，脚本才会继续应用默认设置。

如果默认模型尚未就绪而用户直接提交 prompt，脚本会在发送事件的捕获阶段暂缓这一次提交，完成模型选择后再触发 Gemini 的发送按钮。若模型已正确选中，则不会拦截提交；若模型暂时不可用，也会按当前模型继续发送，避免丢失 prompt。
